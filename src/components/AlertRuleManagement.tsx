...
import type { AlertRule, AISettings } from '../types';
...
export const AlertRuleManagement: React.FC<AlertRuleManagementProps> = ({ alertRules, onDataChange }) => {
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      setFormData({
        ...formData,
        alert_name: response.alert_name || response.name || response.alert || 'GeneratedAlert',
        expr: response.expr || response.expression || 'up == 0',
  ...
  const saveAISettings = async () => {
    try {
      if (aiSettings.id) {
        await aiSettingsService.updateAISettings(aiSettings.id, aiSettings);
      } else {
        await aiSettingsService.createAISettings(aiSettings);
      }
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`px-2 py-1 rounded-full text-xs border ${getSeverityColor('warning')}`}>
                      warning
                    </span>
                  </div>
                </div>
